...
if (isset($_POST['submit_new_acd_rk'])) {
  ...
  if (empty($fileA)) {
    ...
    echo "<script type='text/javascript'>alert('Data Update Successfully!'); window.location.href = 'tab_test.php?id=".$ic."';</script>";
              ...
              if($dbconfig->multi_query( $sql5) == TRUE)
                {
                 ...
                 echo "<script>window.location.assign('tab_test.php?id=$stud_id')</script>";
                }
...
$target_file = $target2 . basename($newfilenameD);
  ...
  $sql1= "UPDATE document SET namaFail = '$filename' where id = '$id'";
    ...
    if ($dbconfig->multi_query($sql1) == TRUE) {
          ...
          echo "<script>alert('Data updated');</script>";
          echo "<script>window.location.assign('tab_test.php?id=$ic')</script>";
    }else
    {
          echo "<script>alert('Tidak dapat diproses');</script>";
    }
     ...
     $sql2= "UPDATE document SET namaFail = '$filename', fileDoc = '$newfilenameD' where id = '$id'";
 ...
 if (isset($_POST['submit_final'])) {
    ...
    while($stud=mysqli_fetch_array($check_stud)){
      ...
      $stud_weight=$stud['weight'];
       ...
       if($stud_name!='' && $stud_nation != '' && $stud_pob != '' && $stud_gender != '' && $stud_dob != '' && $stud_marital != '' && $stud_race != '' )
    {
