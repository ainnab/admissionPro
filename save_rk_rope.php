...
$id = $_REQUEST['id'];
   ...
   $result.= " p1_name='$name1', p1_ic='$ic1', p2_name='$name2', p2_ic='$ic2',household='$household',income ='$income' ";
...
$newfilename=$stud_id."_".$fileA_update;
$target2 = "folder/academic/";
$target_file = $target2 . basename($newfilename);
...
$target2 = "folder/upload/" ;
$target_file = $target2 . basename($newfilenameD);

  $query = "INSERT INTO document (stud_ic,namaFail,fileDoc) VALUES ('$ic','$filename','$newfilenameD')";
...
$newfilenameD=$ic."_".$fileDoc_update;
$target2 = "folder/upload/";
$target_file = $target2 . basename($newfilenameD);
...
if(isset($_GET['deletedoc']))
 ...
 if (isset($_POST['submit_new_certRK'])) {
  ...
  $target = "folder/rope/" .basename($newfilenameD);
  
    $query = "INSERT INTO certificate (stud_ic,certDoc) VALUES ('$ic','$newfilenameD')";
  ...
  $certDoc = $_FILES['certDoc']['name'];
    ...
    $target = "folder/rope/" .basename($newfilenameD);
      ...
      if (empty($certDoc)) {
        ...
        $sql2= "UPDATE certificate SET certDoc = '$newfilenameD' where stud_ic = '$id'";
 ...
 if (isset($_POST['submit_final'])) {
    ...
    $check_cert=mysqli_query($dbconfig,"select * from certificate where stud_ic='$ic'");
    while($cert=mysqli_fetch_array($check_cert)){
      
       $certDoc = $_FILES['certDoc']['name'];
    
    }
    if($stud_name!='' && $stud_nation != '' && $stud_pob != '' && $stud_gender != '' && $stud_dob != '' && $stud_marital != '' && $stud_race != '' && $stud_religion != '' && $stud_height != '' && $stud_weight != '' && $certDoc != '')
    {
